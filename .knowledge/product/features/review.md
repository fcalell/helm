...
When a run finishes, a cheap review session (read-only allowlist) grades each acceptance criterion
from the story's brief: **✓ / ✗ / unclear, each with evidence** (file:line references, test or
command output). The tally lands on the card (5/6 ✓). Grades are claims to verify, never
...
- **Approve**: merge to main (or open a PR via `gh` in PR mode), delete worktree + branch, card →
  Done.
