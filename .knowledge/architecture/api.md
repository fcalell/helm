...
| `review.requestChanges` | Turns the user's comments (`{ criterion?, text }[]`, at least one) into the next message in the same run session and worktree: requires status `review` whose last run entry closed `outcome: review`, resumes that entry's session with the comments (per-criterion ones quote the criterion) and a notice that the branch was rebased at the close, spawned with `--model fable --effort high` (the escalation case, [session-kinds](./session-kinds.md) §Model per kind). The init write reopens the entry (`outcome`, `error`, `stat` cleared) and flips the card to Running; the follow-up closes through the unchanged close path, usage summing onto the same entry. Same `{ sessionId } \| { queued: true }` union as `run.start`; an empty comment list is `BAD_REQUEST`. |
