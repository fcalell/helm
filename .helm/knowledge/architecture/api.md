...
| `story.move` | Re-reads the story from disk (the snapshot only resolves id → path), validates the transition with `canTransition`, writes the new status, and returns `{ gating: false } \| { gating: true, phase }` (`phase` is the held attempt's gate phase, never `exhausted`: retrying an exhausted attempt re-queues it); the `board` channel's snapshot is the authority. Serialized through a per-repo write queue. A move into `ready` runs the ready gate: an incomplete brief is refused, a valid recorded `gate` verdict (frontmatter hash matches the brief body) writes `ready` for free, and a `refining` story otherwise enqueues a cold adversary pass on the dispatcher and returns `gating: true` with its phase and the card unchanged; the client toasts that phase. The round then streams on the `gate` channel. A move into `running` is always refused: stories enter `running` through `run.start` alone. |
| `run.start` | Starts an implementation run on a Ready story: validates `ready → running` plus gate freshness (a stale verdict is refused), then dispatches through the serial run queue. With the slot free it converges the story's branch + worktree, spawns the run session under the story's permission preset with the brief as the per-segment system-prompt seed ([claude-integration](./claude-integration.md) §Context management), writes `status: running` with a new `runs` entry once `system/init` lands, and returns `{ sessionId }`; with the slot held it returns `{ queued: true }` at once and the run spawns when the slot frees (fresh starts in start-request order). The dequeue re-runs validation: a story that went stale while waiting, or a dequeue-time spawn failure, is skipped with a `"run-skipped"` board notice and the queue advances. An invalid `.helm/config.json` refuses the start (`INVALID_FILE`) before any spawn. The run streams on the `session` channel and the close path flips the card to Review or Blocked. A close that would land Review first rebases the story branch onto the repo's main branch, captures the configured check command's result to `<storyId>.check.json` (with a Conventional Commit lint of the branch's commits), and writes the diff stat onto the closing entry ([board-storage](./board-storage.md) §Worktrees); a rebase conflict aborts the rebase (worktree left on the pre-rebase tip) and parks the card Blocked with an error naming the rebase. One run per story at a time (`RUN_ACTIVE` while one is live or queued); the run holds the queue slot from spawn to process close, so a paused or asking story holds none. |
...
| `review.approve` | Approves a Review card: rebases the story branch onto main in the worktree (main may have moved since the close), fast-forward-merges it into the managed checkout's main (refused when the checkout sits on another branch), pushes when main has an upstream, then deletes the worktree, the branch, and the story's per-worktree artifacts and writes `status: done`. Returns `{ pushed, pushError? }`: the push is best-effort, so a missing upstream or a failed push completes the exit with `pushed: false` and the merge intact. Any git failure before the merge rejects `EXIT_FAILED` with the card unchanged; after the merge, push and cleanup failures are logged, never parked. Requires status `review` with no live or queued run (`ILLEGAL_TRANSITION` / `RUN_ACTIVE`) and an existing worktree (`NOT_FOUND`). |
| `review.requestChanges` | Turns the user's comments (`{ criterion?, text }[]`, at least one) into the next message in the same run session and worktree: requires status `review` whose last run entry closed `outcome: review`, resumes that entry's session with the comments (per-criterion ones quote the criterion) and a notice that the branch was rebased at the close, spawned with `--model opus --effort high` (the model follows the `run` row so the resume keeps its warm cache; high is the escalation, [session-kinds](./session-kinds.md) §Model per kind). The init write reopens the entry (`outcome`, `error`, `stat` cleared) and flips the card to Running; the follow-up closes through the unchanged close path, usage summing onto the same entry. Same `{ sessionId } \| { queued: true }` union as `run.start`; an empty comment list is `BAD_REQUEST`. |
| `review.discard` | Discards the attempt: deletes the worktree, the story branch (`-D`, it is unmerged), and the per-worktree artifacts, tolerating whatever is already gone, then writes `status: ready`. The closed run entry stays (outcome, `stat`, tokens: the history is the point) and the gate still holds, so the next `run.start` appends a fresh entry on a fresh worktree. Same preconditions as approve minus the worktree requirement; a git deletion failure rejects `EXIT_FAILED` with the card unchanged. |
